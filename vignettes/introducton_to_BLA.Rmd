...
The package `aplpack` has also been loaded for outlier detection function `bagplot()`.
...
An exploratory analysis is an important initial step in boundary line analysis. This step serve to show how the data is distributed using various indices and also check for outliers in the data set. If a variable in a dataset is skewed, especially in the independent variable, a transformation may be required so it can be assumed to be from a normal distribution. As the boundary line analysis is sensitive to outlying values, these must be identified and removed if required. The response variable, in this case P, you may expect to see a spike at the upper section of the distribution because of the clustering points but the rest of the data points must be normally distributed. Exploratory analysis also provides an objective test to check if the data meets the criteria for boundary line analysis as well as the form of model that can be fitted.

### Normality test

To check for the distribution of the data, the *`summa()`* function provides different indices which can be used to check for distribution normality of a given parameter. The function *`summaplot()`* give a graphical representation of the parameter distribution. Lets set the variables evapotranspiration and wheat yield in the data set to *x* and *y* respectively before applying the *`summa()`* and *`summaplot()`* functions
...
From the results, wheat yield (*y*) variables can be considered to be normally distributed while the soil P (*x)* is skewed. For a variable to be assumed to be from a normal distribution, the indices skewness should be between -1 to 1 and Octile skewness between -0.2 to 0.2. A graphical representation of the distribution can be checked using
...
Outliers are identified using the bagplot method. A bag plot is a multivariate equivalent of a box plot. The bagplot has three main components. There include
...
bag<-bagplot(vals_ur, ylim=c(0,20), show.whiskers =F) # run the bagplot function
...
To check if the data constitutes a boundary, the function *`exp_boundary()`* can be used. This function determines evidence of boundary in data set when compared against an unbounded bivariate normally distributed data. The standard deviation (*sd*) of the the euclidean distance of the boundary points to the center of the data set is used to measure the density of the points at the upper edges of the data. The smaller the *sd* value, the denser the distribution. This function uses the principle of convex hull to select the points at the upper boundary. The default is selecting the first 10 consecutive convex hulls (*shells=10*). The convex hulls are then splits into two sections, the right and left sections, and evidence of boundary existence in both sections is checked by determining the probability of having the observed density of points at the upper edges of the data given that it is not bounded. We take 0.05 probability as the significance level.
...
With a scatter plot of *`y`* against *`x`* active in the plot window in `R`, run the function *`startValues("trapezium")`*, then click on the plot, five boundary points that follow the trapezium model in order of increasing *`x`* values.
...
The binning methodology involves splitting the data into several sections in the x-axis and selecting a boundary point in each section based on a set criteria (mostly the 95$^{th}$ and 99$^{th}$ percentile) (Shatar and McBratney, 2004). To fit the boundary line using the binning method, the *`blbin()`* function can be used. To check the required arguments for the function, the help page can be launched.

```{r, eval=FALSE}
?blbin
```

The arguments *`x`* and *`y`* are the independent and dependent variable respectively and *theta* is a vector of starting values . The *`model`* argument is used to specify the model of the boundary line e.g. "blm" for the linear model. The *`bins`* argument describes the size of the bins with a vector of length 3 containing the minimum and maximum independent variable values, and the size of bins to be used for the data respectively. We assume that the 99$^{th}$ percentile (`tau=0.99`) is the boundary.
...
The arguments *`x`* and *`y`* are the independent and dependent variable respectively and *`theta`* is a vector of starting values . The *`model`* argument is used to specify the model of the boundary line e.g. "blm" for the linear model. The argument *`tau`* describes the quantile value described as boundary. We assume that the 99$^{th}$ quantile (`tau=0.99`) value is the boundary.
...
To fit the boundary line using the Censored bivariate normal model method, see the vignette, *"Yield gap analysis using the censored bivariate normal model"*
...
1.  Cade, B. S., & Noon, B. R. (2003). A gentle introduction to quantile regression for ecologists. Frontiers in Ecology and the Environment, 1(8), 412-420.

2.  Fermont, A. M., Van Asten, P. J., Tittonell, P., Van Wijk, M. T., & Giller, K. E. (2009). Closing the cassava yield gap: an analysis from smallholder farms in east africa. Field Crops Research, 112 (1), 24--36.

3.  Lark, R. M., Gillingham, V., Langton, D., & Marchant, B. P. (2020). Boundary line models for soil nutrient concentrations and wheat yield in national-scale datasets. European Journal of Soil Science, 71 , 334-351. doi: <https://doi.org/10.1111/>[ejss.12891](https://doi.org/10.1111/ejss.12891)

4.  Milne, A. E., Wheeler, H. C., & Lark, R. M. (2006). On testing biological data for the presence of a boundary. Annals of Applied Biology, 149 , 213-222. doi:<https://doi.org/10.1111/j.1744-7348.2006.00085.x>

5.  Shatar, T. M., & McBratney, A. B. (2004). Boundary-line analysis of field-scale yield response to soil properties. Journal of Agricultural Science, 142 , 553-560. doi: <https://doi.org/10.1017/S0021859604004642>

6.  Schnug, E., Heym, J. M., & Murphy, D. P. L. (1995). Boundary line determination technique (bolides). In P. C. Robert, R. H. Rust, & W. E. Larson (Eds.), site specific management for agricultural systems (p. 899-908). Wiley Online Library. doi: <https://doi.org/10.2134/1995.site-specificmanagement.c66>

7.  Webb, R. A. (1972). Use of the boundary line in analysis of biological data. Journal of Horticultural Science, 47, 309--319.
