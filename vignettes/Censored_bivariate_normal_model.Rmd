...
sigh=c(0.5,0.7,0.8)
ble_profile(vals,start,sigh,model = "lp") # may take a few minutes to run for large datasets
```
...
model1<-cbvn(vals,start=start,sigh=0.7,model = "lp", xlab=expression("P /log ppm"), ylab=expression("Yield /t ha"^{-1}), pch=16, col="grey")
...
model2<-cbvn(vals,start=start,sigh=0.7,model = "lp", xlab=expression("pH"), ylab=expression("Yield /t ha"^{-1}), pch=16, col="grey")
