...
The `BLA` package is loaded together with the `aplpack` package. The `aplpack` package contains the `bagplot()` function which is used to identify outliers in a data set.
...
df<-data.frame(x,y) 
bag<-bagplot(df,show.whiskers = FALSE, ylim=c(0,20),create.plot = TRUE)
...
model1<-cbvn(vals,theta=theta,sigh=0.7,model = "lp", xlab=expression("P /log ppm"), ylab=expression("Yield /t ha"^{-1}), pch=16, col="grey")
...
model2<-cbvn(vals,theta=theta,sigh=0.7,model = "lp", xlab=expression("pH"), ylab=expression("Yield /t ha"^{-1}), pch=16, col="grey")
