...
To assess the distribution of the data, the `summa()` function provides different summary statistics. The function `summaplot()` give a graphical representation of the variable distribution. Let us set the variables P and wheat yield in the data set to *`x`* and *`y`* respectively before applying the `summa()` and `summaplot()` functions
...
```{r,fig.align='center', fig.dim=c(4,4), eval=FALSE, echo=FALSE}
summaplot(x) 
summaplot(y) 
```
...
Outliers are identified using the bagplot method (Rousseeuw *et al*., 1999. A bag plot is a multivariate equivalent of a box plot. We utilise the `bagplo()` function from the `aplpack` package to produce bagplots.The bagplot has three main components. There are
...
bag<-bagplot(vals_ur, ylim=c(0,20), show.whiskers =F,create.plot = FALSE) # run the bagplot function
