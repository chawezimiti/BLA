...
[`vignette("Introduction_to_BLA")`](https://chawezimiti.github.io/BLA/articles/Introduction_to_BLA.md).
...
boundary model in the `BLA` package, encoded in the functions
...
Additionally, the `BLA` provides function for initial data exploratory
...
boundary line models when using
[`cbvn()`](https://chawezimiti.github.io/BLA/reference/cbvn.md). These
include (1) checking the distribution of the potential limiting and
response variables to access if they fulfill the assumption of
normality, (2) detection of outliers, and (3) the determination of
evidence of boundary existence in a dataset.
...
use the [`cbvn()`](https://chawezimiti.github.io/BLA/reference/cbvn.md)
function to fit the boundary line.
...
features as in the function
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) from base `R`.
...
function. Set the argument `model` to the desired model
...
can be predicted using the function
